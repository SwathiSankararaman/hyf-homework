...
function shortestWord(danishWords) {
    ...
    for (let i = 0; i < danishWords.length; i++) {
        lengthOfDanishWords.push(danishWords[i].length);
    }
    let x = Math.min(...lengthOfDanishWords); // This function helps to calculate min value in the array length and store in variable x
    for (let j = 0; j < danishWords.length; j++) { // This for loop iterates over the length array and checks with x
        if (x === lengthOfDanishWords[j]) {
