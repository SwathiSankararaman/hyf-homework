...
const shortMovies = movies.filter(mov => mov.title.length < 3);
console.log(shortMovies);

// Create an array of movie titles with long movie titles

const longMovies = movies.filter(mov => mov.title.length > 50);
console.log(longMovies);

// Count the number of movies made between 1980-1989 (including both the years)

const eightiesMovies = movies.filter(mov => (mov.year >= 1980 && mov.year <= 1989));
console.log(eightiesMovies.length);

// Create a new array that has an extra key called tag. The tag is based on the rating: Good (>= 7), 
// Average (>= 4 and < 7), Bad (< 4)

let moviesWithTag = movies.map(mov => {
   if (mov.rating >= 7) {
      mov.tag = 'Good';
   } else if (mov.rating >= 4 && mov.rating < 7) {
      mov.tag = 'Average';
   } else {
      mov.tag = 'Bad';
   }
...
let averageRating = movies.reduce((averageRating, value) => averageRating + value.rating, 0) / movies.length;
console.log(Math.round(averageRating));


// Count the total number of Good, Average and Bad movies using reduce. Optional
// Here I am having accumulator as an object with keys good, average and bad so that I can store the count as the value

const filtered = moviesWithTag.reduce((acc, value) => {
   if (value.tag === 'Good') {
      acc.Good += 1;
   }
   else if (value.tag === 'Average') {
      acc.Average += 1;
   }
   else if (value.tag === 'Bad') {
      acc.Bad += 1;
   }
   // acc[value.tag] += 1;
   return acc;
...
console.log(`The count of Good movies: ${filtered.Good}\n The count of Good movies: ${filtered.Average}\n The count of Good movies: ${filtered.Bad}`);
