...
function getReply(command) {
    ...
    if (splitCommand[0] == 'Hello') {
        ...
        // Question 6 (What is on my todo?)
        if (splitCommand.includes('todo') || splitCommand.includes('todo?')) {
        ...
        if (splitCommand.includes('+') || splitCommand.includes('-') || splitCommand.includes('*') || splitCommand.includes('/')) {
            ...
            x = parseInt(x, 10); // Converts string to whole integer(In our case '3' to 3)
            y = parseInt(y, 10);
            if (operator == '+') {
                let sum = x + y;
                return sum;
        ...
        // Question 10 (What is my favorite dish)
        if (splitCommand.includes('favorite') && (splitCommand.includes('dish') || splitCommand.includes('dish?'))) {
            return favouriteDish;

        }


        // Question 13 (What am I doing this week?)
        if (splitCommand.includes('doing') && (splitCommand.includes('week') || splitCommand.includes('week?'))) {
